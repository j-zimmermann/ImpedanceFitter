...
model1 = 'R_f0 + parallel(R_ct + W, C)'
model2 = 'R_f0 + parallel(R_ct + W, CPE)'


def test_randles_type():
    Z = Z_randles(omega, R0, Rs, Aw, C0)
...
def test_randles_CPE_type():
    Z = Z_randles_CPE(omega, R0, Rs, Aw, k, alpha)
    assert isinstance(Z, np.ndarray)
...
def test_randles_model():
    ...
    Z_m = m.eval(omega=omega, ct_R=R0, f0_R=Rs,
                 C=C0, Aw=Aw)
