...
def convert_diel_properties_to_impedance(omega, eps_r, sigma, c0):
    ...
    Note that the vacuum permittivity :math:`\varepsilon_0` is contained in :math:`c_0`.
...
def return_diel_properties(omega, Z, c0):
    ...
    a direct mapping from impedance to relative complex permittivity is possible:
...
def return_dielectric_modulus(omega, Z, c0):
    ...
    .. [Bordi2001] Bordi, F., & Cametti, C. (2001).
                   Occurrence of an intermediate relaxation process in
                   water-in-oil microemulsions below percolation:
    ...
    for p in bufdict:
        ...
        if par in zerotoones:
            if not 0 <= bufdict[p].value <= 1.0:
                raise ValueError(
                    f"{p} is an exponent or the volume fraction that needs"
                    " to be between 0.0 and 1.0. Change your initial value."
                )
            if bufdict[p].vary:
...
def set_parameters(model, parameterdict=None, emcee=False, weighting_model=False):
    ...
    parameterdict: dict, optional
        A dictionary containing parameters for model with
        *min*, *max*, *vary* info for LMFIT.
...
def get_labels(params):
    ...
    for p in params:
        ...
        try:
            ...
            print(
                "There has not yet been a LaTex representation "
                f"of the parameter {par} implemented."
            )
        ...
        if par in baseparams:
            if len(suffixes[par]) == 0 or suf is None:
                raise RuntimeError(
                    ...
                    "Please make sure that all parameters of this type have "
                    "a proper prefix according to its model's suffix. "
...
def make_eps(omega, c0all):
    """Convert capacitance to permittivity."""
...
def _cycle_circuit(circuit, d, endpts, step=3.0, depth=0):
    ...
    endpts:
        SchemDraw end points.
    step:
        Step for SchemDraw
    depth:
        Current level in ECM
...
def KK_integral_transform(omega, Z):
    ...
    for i, w in enumerate(omega):
        ...
        ZKK[i] = -2.0 / np.pi * simps(integrand, x=x)
        # imag part
        integrand = (real - Z.real[i]) / (x * x - w * w)
        ZKK[i] += 1j * 2.0 * w / np.pi * simps(integrand, x=x)
    return ZKK
